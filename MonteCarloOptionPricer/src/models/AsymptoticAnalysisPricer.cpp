...
// Used for asymptotic analysis in pricing
#include "../../include/core/common.h"  // For PayoffFunction(...) 
#include <cmath>
#include <algorithm>
#include <numeric>
#include <stdexcept>
...
/**
 * For American Call with dividend if D > r.
 */
    ...
    double dividend)
{
    ...
    // Validate all paths have same length
    for(int i = 0; i < N; ++i) {
        if(pricePaths[i].size() != size_t(M)) {
            return 0.0;  // Return 0 for malformed paths
        }
    }
    ...
    try {
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < M; ++j) {
                ...
                if (std::isnan(S) || std::isinf(S)) continue;  // Skip invalid prices
