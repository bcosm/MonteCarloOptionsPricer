...
int main()
{
    try {
        ...
        auto [predMean, ciLower, ciUpper] = loaded_trainer.MetaModelPrediction(newFeatures, /*nSamples=*/1);
        ...
        {
            loaded_trainer.LoadModel(model_file);
            loaded_trainer.MetaModelPrediction(newFeatures, /*nSamples=*/1);
            loaded_trainer.MetaModelPrediction(newFeatures, /*nSamples=*/1);

            double sumVals = 0.0;
            double sumSq   = 0.0;
            for(int64_t i=0; i<mcSamples; ++i){
                auto [tmp, lower, upper] = loaded_trainer.MetaModelPrediction(newFeatures, /*nSamples=*/1);
                sumVals += tmp;
                sumSq   += (tmp * tmp);
            }
        ...
        for(size_t i=0; i<X_valid.size(); ++i){
            auto [pred, lower, upper] = loaded_trainer.MetaModelPrediction(X_valid[i], /*nSamples=*/1);
        ...
        for(int64_t i=0; i<X_test.size(); ++i){
            auto [pred, lower, upper] = loaded_trainer.MetaModelPrediction(X_test[i], /*nSamples=*/1);
